...
- 入口：
  ...
  - 详情页：收藏（RSS），以及平台侧动作（B站点赞/投币/收藏，现状：占位）
- 行为约定：
  - “收藏/稍后再看”在应用内统一沉淀为本地列表（现状：仅 RSS）
  - B站侧“稍后再看/历史记录”与应用内列表：首版以“尽量同步、失败不阻塞使用”为原则（现状：占位）
