...
- 首页（入口页，展示各 channel）：
  - 顶部显示个人头像（现状：未接入欢太账号，使用占位）(点击后进入"我的"页面)；
  - 中部为内容入口（B站/抖音/订阅的 RSS channel，现状：B站/抖音入口为占位），点击进入内容页
  - 底部:添加RSS
  - 本页内容的菜单:
    - 快速操作菜单:
      - 移动到顶部/标记为已读
...
- 详情页：
  - 对于标准RSS:
    - 顶部显示标题
    - 标题和正文中间显示“打开原文”（(如果提供了item:link)）
    - 中部为内容正文； 
    - 底部显示收藏和分享按钮，这俩按钮是圆形图标按钮
  - 对于B站（现状：已实现）:
    - 视频标题
    - 中部显示视频封面,点击进入播放页
    - 作者信息
...
- **手势操作**：
  - 上滑/下滑：页面滚动/翻页
  - 左滑：
    - 滑动项目调出快速操作菜单 (统一为滑动后出现按钮,点击按钮执行操作)（参考IOS上的微信/QQ的滑动联系人条目交互）（按钮横向排布）
    - 空白处滑动无动作
  - 右滑：返回上一级
...
- 现状：[已实现] 已接入登录/内容/播放与基础动作
- 触发：首次进入 B站 channel 或执行需要鉴权的动作（点赞/收藏/投币/同步稍后再看等）,或手动登陆(按钮放在channel详情页中)
  
- 登录流程：
  - 打开登录页（SDK 二维码登录 + WebView 密码登录备选）
  - 登录成功：保存 cookies，返回 B 站内容页并触发刷新
...
## 3.6 收藏/稍后再看（统一能力）
- 入口：
  - 内容页左滑快速菜单：收藏/稍后再看（现状：仅 RSS）
  - 详情页：收藏（RSS），以及平台侧动作（B站点赞/投币/收藏，现状：已实现）
- 行为约定：
  - “收藏/稍后再看”在应用内统一沉淀为本地列表（现状：仅 RSS）
  - B站侧“稍后再看/历史记录”与应用内列表：首版以“尽量同步、失败不阻塞使用”为原则（现状：已接入 B站侧列表）
...
# 4.首个正式版功能列表
  ...
  - 正文强制阅读模式,显示纯文本和图/视频（现状：已做 HTML 解析为内容块）
    - 图片设定max-width但不设高度限制
    - 视频显示一个封面+播放图标,点击后跳转播放器页面
  - 支持缓存
  ## B站（现状：[已实现] App 侧已接入）
  - 支持视频播放（已实现，MP4 播放 + 浏览器兜底）
  - 支持登录和个性化推荐（已实现）
  - 支持点赞/收藏/投币（已实现）
  - 支持B站账号同步稍后再看/历史记录（已实现）
  - 播放页面功能同步鸿蒙版本,设计使用OPPO或谷歌规范（规划）
  
  ## 抖音（现状：[占位] App 侧未接入）
  - 支持账号登录和个性化视频推送获取（规划）
  - 支持视频播放（规划）
  - 播放页面为竖屏及短视频上下滑动切换单独搭建和优化（规划）
  ## 其他功能
  - 应用内的统一的收藏和稍后再看功能（现状：仅 RSS，B站使用平台侧收藏/稍后再看）
  - 阅读页主题切换与字体大小调节
...
- 未读呈现为title行最右侧一个圆点
## 应用内收藏/稍后再看系统
- 对象:抖音/B站视频,RSS item（现状：仅 RSS，B站使用平台侧列表）
- 被收藏/稍后再看的视频需要存链接,但不下载内容到本地（规划）
...
| 数据存储 | 本地：SharedPreferences（配置信息）<br>云端：OPPO云服务（规划） | 敏感信息加密存储，仅同步用户主动开启的内容，v1不做云同步 |
